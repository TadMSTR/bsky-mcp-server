...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/bsky-mcp-server/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[bsky-mcp-server] Security Report`.
...
- SSRF via the Bluesky ATP/PDS URL configuration
- ATP session token exposure through error messages or response logging
- Injection in Bluesky post content, search queries, or DID resolution paths
- Dependency vulnerabilities with a plausible exploitation path in bsky-mcp-server's usage
